<script lang="ts">
  ...
  async function handleCreateTicket(e: Event) {
    ...
    isSubmitting = true;
    try {
      const res = await apiPost<any>('/student/tickets', {
        title: newTicketTitle,
        description: newTicketDesc
      });
      // Try to add to top of list if successful
      if (res && res.id) {
        tickets = [res, ...tickets];
      }
      ...
      <div class="modal-content" onclick={e => e.stopPropagation()} role="dialog" tabindex="-1">
        ...
        <form class="modal-form" onsubmit={handleCreateTicket}>
          <div class="form-group">
            ...
            <input id="title" type="text" bind:value={newTicketTitle} required placeholder="e.g. Cannot access course materials" />
          </div>
          <div class="form-group">
